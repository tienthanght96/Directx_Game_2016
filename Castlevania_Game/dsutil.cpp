	...
	DWORD dwCoopLevel)
{
	...
	if (FAILED(hr = DirectSoundCreate8(NULL, &m_pDS, NULL)))
		return DXTRACE_ERR(TEXT(L"DirectSoundCreate8"), hr);

	// Set DirectSound coop level 
	if (FAILED(hr = m_pDS->SetCooperativeLevel(hWnd, dwCoopLevel)))
		return DXTRACE_ERR(TEXT(L"SetCooperativeLevel"), hr);
	...
	DWORD dwPrimaryBitRate)
{
	...
	if (FAILED(hr = m_pDS->CreateSoundBuffer(&dsbd, &pDSBPrimary, NULL)))
		return DXTRACE_ERR(TEXT(L"CreateSoundBuffer"), hr);
	...
	if (FAILED(hr = pDSBPrimary->SetFormat(&wfx)))
		return DXTRACE_ERR(TEXT(L"SetFormat"), hr);
	...
	DWORD dwNumBuffers)
{
	...
	if (FAILED(hr))
	{
		...
		DXTRACE_ERR(TEXT(L"CreateSoundBuffer"), hr);
	...
	if ((dwCreationFlags & DSBCAPS_CTRLFX) == 0)
	{
		for (i = 1; i < dwNumBuffers; i++)
		{
			...
			{
				DXTRACE_ERR(TEXT(L"DuplicateSoundBuffer"), hr);
				goto LFail;
			}
		}
	}
...
HRESULT CSound::FillBufferWithSound(LPDIRECTSOUNDBUFFER pDSB, BOOL bRepeatWavIfBufferLarger)
{
	...
	if (FAILED(hr = RestoreBuffer(pDSB, NULL)))
		return DXTRACE_ERR(TEXT(L"RestoreBuffer"), hr);

	// Lock the buffer down
	if (FAILED(hr = pDSB->Lock(0, m_dwDSBufferSize,
		&pDSLockedBuffer, &dwDSLockedBufferSize,
		NULL, NULL, 0L)))
		return DXTRACE_ERR(TEXT(L"Lock"), hr);
	...
	if (FAILED(hr = m_pWaveFile->Read((BYTE*)pDSLockedBuffer,
		...
		&dwWavDataRead)))
		return DXTRACE_ERR(TEXT(L"Read"), hr);
		...
		if (bRepeatWavIfBufferLarger)
		{
			...
			while (dwReadSoFar < dwDSLockedBufferSize)
			{
				...
				if (FAILED(hr = m_pWaveFile->ResetFile()))
					return DXTRACE_ERR(TEXT(L"ResetFile"), hr);
...
HRESULT CSound::RestoreBuffer(LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored)
{
	...
	if (FAILED(hr = pDSB->GetStatus(&dwStatus)))
		return DXTRACE_ERR(TEXT(L"GetStatus"), hr);
...
HRESULT CSound::Play(DWORD dwPriority, DWORD dwFlags, LONG lVolume, LONG lFrequency, LONG lPan)
{
	...
	if (pDSB == NULL)
		return DXTRACE_ERR(TEXT(L"GetFreeBuffer"), E_FAIL);
	...
	if (bRestored)
	{
		...
		if (FAILED(hr = FillBufferWithSound(pDSB, FALSE)))
			return DXTRACE_ERR(TEXT(L"FillBufferWithSound"), hr);
	}
...
HRESULT CSound::Play(LONG Volume , LONG lFrequency)
{
	HRESULT hr;
	BOOL    bRestored;
	DWORD dwPriority = 0; DWORD dwFlags = 0; LONG lPan = 0;
	LONG lVolume = Volume;
	if (m_apDSBuffer == NULL)
		return CO_E_NOTINITIALIZED;
...
HRESULT CWaveFile::Open(LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags)
{
	...
	if (m_dwFlags == WAVEFILE_READ)
	{
		...
		if (NULL == m_hmmio)
		{
			...
			if (NULL == (hResInfo = FindResource(NULL, strFileName, TEXT("WAVE"))))
			{
				if (NULL == (hResInfo = FindResource(NULL, strFileName, TEXT("WAV"))))
					return DXTRACE_ERR(TEXT(L"FindResource"), E_FAIL);
			}

			if (NULL == (hResData = LoadResource(NULL, hResInfo)))
				return DXTRACE_ERR(TEXT(L"LoadResource"), E_FAIL);

			if (0 == (dwSize = SizeofResource(NULL, hResInfo)))
				return DXTRACE_ERR(TEXT(L"SizeofResource"), E_FAIL);

			if (NULL == (pvRes = LockResource(hResData)))
				return DXTRACE_ERR(TEXT(L"LockResource"), E_FAIL);
		...
		if (FAILED(hr = ReadMMIO()))
		{
			...
			mmioClose(m_hmmio, 0);
			return DXTRACE_ERR(TEXT(L"ReadMMIO"), hr);
		}
	...
	else
	{
		m_hmmio = mmioOpen(strFileName, NULL, MMIO_ALLOCBUF |
			...
			MMIO_CREATE);
		if (NULL == m_hmmio)
			return DXTRACE_ERR(TEXT(L"mmioOpen"), E_FAIL);

		if (FAILED(hr = WriteMMIO(pwfx)))
		{
			mmioClose(m_hmmio, 0);
			return DXTRACE_ERR(TEXT(L"WriteMMIO"), hr);
		}
...
HRESULT CWaveFile::ReadMMIO()
{
	...
	if ((0 != mmioDescend(m_hmmio, &m_ckRiff, NULL, 0)))
		return DXTRACE_ERR(TEXT(L"mmioDescend"), E_FAIL);

	// Check to make sure this is a valid wave file
	if ((m_ckRiff.ckid != FOURCC_RIFF) ||
		(m_ckRiff.fccType != mmioFOURCC('W', 'A', 'V', 'E')))
		return DXTRACE_ERR(TEXT(L"mmioFOURCC"), E_FAIL);
	...
	if (ckIn.cksize < (LONG) sizeof(PCMWAVEFORMAT))
		return DXTRACE_ERR(TEXT(L"sizeof(PCMWAVEFORMAT)"), E_FAIL);

	// Read the 'fmt ' chunk into <pcmWaveFormat>.
	if (mmioRead(m_hmmio, (HPSTR)&pcmWaveFormat,
		sizeof(pcmWaveFormat)) != sizeof(pcmWaveFormat))
		return DXTRACE_ERR(TEXT(L"mmioRead"), E_FAIL);
	...
	if (pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM)
	{
		...
		if (NULL == m_pwfx)
			return DXTRACE_ERR(TEXT(L"m_pwfx"), E_FAIL);
	...
	else
	{
		...
		if (NULL == m_pwfx)
			return DXTRACE_ERR(TEXT(L"new"), E_FAIL);
	...
	{
		SAFE_DELETE(m_pwfx);
		return DXTRACE_ERR(TEXT(L"mmioAscend"), E_FAIL);
	}
	...
	else
	{
		...
		if (m_dwFlags == WAVEFILE_READ)
		{
			...
			if (-1 == mmioSeek(m_hmmio, m_ckRiff.dwDataOffset + sizeof(FOURCC),
				SEEK_SET))
				return DXTRACE_ERR(TEXT(L"mmioSeek"), E_FAIL);
		...
		else
		{
			...
			if (0 != mmioCreateChunk(m_hmmio, &m_ck, 0))
				return DXTRACE_ERR(TEXT(L"mmioCreateChunk"), E_FAIL);

			if (0 != mmioGetInfo(m_hmmio, &m_mmioinfoOut, 0))
				return DXTRACE_ERR(TEXT(L"mmioGetInfo"), E_FAIL);
		}
	}
...
HRESULT CWaveFile::Read(BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead)
{
	...
	else
	{
		...
		for (DWORD cT = 0; cT < cbDataIn; cT++)
		{
			...
			if (mmioinfoIn.pchNext == mmioinfoIn.pchEndRead)
			{
				if (0 != mmioAdvance(m_hmmio, &mmioinfoIn, MMIO_READ))
					return DXTRACE_ERR(TEXT(L"mmioAdvance"), E_FAIL);

				if (mmioinfoIn.pchNext == mmioinfoIn.pchEndRead)
					return DXTRACE_ERR(TEXT(L"mmioinfoIn.pchNext"), E_FAIL);
			}
		...
		if (0 != mmioSetInfo(m_hmmio, &mmioinfoIn, 0))
			return DXTRACE_ERR(TEXT(L"mmioSetInfo"), E_FAIL);
...
HRESULT CWaveFile::WriteMMIO(WAVEFORMATEX *pwfxDest)
{
	...
	if (pwfxDest->wFormatTag == WAVE_FORMAT_PCM)
	{
		if (mmioWrite(m_hmmio, (HPSTR)pwfxDest,
			sizeof(PCMWAVEFORMAT)) != sizeof(PCMWAVEFORMAT))
			return DXTRACE_ERR(TEXT(L"mmioWrite"), E_FAIL);
	}
	else
	{
